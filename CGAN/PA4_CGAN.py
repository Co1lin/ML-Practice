...
def save_image(img, path, nrow=10, padding=5):
    ...
    if C==3:
        ...
        img = img[:,:,0]
    Image.fromarray(np.uint8(img)).save(f'{dir_path}/{path}')
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        gen_labels = labels # jt.array(np.random.randint(0, opt.n_classes, batch_size)).float32()
    ...
    if epoch % 10 == 0:
        generator.save(f"{dir_path}/generator_last.pkl")
        discriminator.save(f"{dir_path}/discriminator_last.pkl")

generator.eval()
discriminator.eval()
generator.load(f'{dir_path}/generator_last.pkl')
discriminator.load(f'{dir_path}/discriminator_last.pkl')

number = '2019013266' # TODO: 写入你的学号（字符串类型）
n_row = len(number)
...
Image.fromarray(np.uint8(img_array)).save(f"{dir_path}/result.png")
